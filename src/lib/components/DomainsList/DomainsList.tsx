...
import { domainsSelector } from '@/lib/store/domains/domains.selectors';
import { addToCart, setFilteredData } from '@/lib/store/domains/domains.slice';
import { addItemsToCart } from '@/lib/store/cart/cart.slice';
import { Domain } from '@/lib/store/domains/domains.types';
import useFilters, {Sorts as Params} from '@/lib/hooks/useSort';
import { SortData, sortsData } from '@/lib/static';
...
const DomainsList = () => {
  ...
  const domains = useAppSelector(domainsSelector);
  const [sorts, setSorts] = useState(sortsData);
  const [localData, setLocalData] = useState(domains);
  const searchParams = useSearchParams();
  const filter = useFilters(domains)

  useEffect(() => {
    if (searchParams) {
      const currentParams = Object.fromEntries([...searchParams.entries()]) as unknown as Params
      const data = filter(currentParams)
      setSorts(prevstate => [...prevstate.map(item => ({
        ...item,
        sortType: item.sortName === currentParams.sortName ? currentParams.sortType : '' as ''
      }))])
      setLocalData(data)
      dispatch(setFilteredData({data}))
    }
  }, [searchParams, filter, dispatch])

  const handleItemAddToCart = (domain: Domain) => {
    dispatch(addToCart({id: domain.id}));
    dispatch(addItemsToCart({item: domain}))
  }

  const handleSortClick = useCallback((sort: SortData) => {
    const current = new URLSearchParams(Array.from(searchParams.entries()));
    const sortName = current.get('sortName');
    const sortType = current.get('sortType');

    if (sortName === sort.sortName) {
      if (sortType === 'asc') {
        current.set('sortType', 'desc')
      } else {
        current.set('sortType', 'asc')     
      }  
    } else {
      current.set('sortName', sort.sortName);
      current.set('sortType', 'asc')
    }
